...
def corregir_chapa_detectada(chapa_detectada, cursor):
    try:
        ...
        if chapa_detectada in correcciones_manual:
            ...
            logging.info(f"Correccion manual aplicada: {chapa_detectada} => {chapa_corregida}")
        ...
        if min_distance <= 1:
            logging.info(
                f"Chapa corregida: {chapa_detectada} => {chapa_mas_cercana} (distancia: {min_distance})"
            )
            return chapa_mas_cercana

        logging.info(f"No se encontro coincidencia aceptable para {chapa_detectada}")
